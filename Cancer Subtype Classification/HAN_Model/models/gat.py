...
class HeteGAT_multi(BaseGAttN):
    def inference(inputs_list, nb_classes, nb_nodes, training, attn_drop, ffd_drop,
        ...
        final_embed, att_val = layers.SimpleAttLayer(
            multi_embed, mp_att_size, time_major=False, return_alphas=True
        )

        out = []
        for _ in range(n_heads[-1]):
            # CORRECTED: instantiate Dense THEN call it on final_embed
            out.append(
                Dense(units=nb_classes, activation=None)(final_embed)
            )
            ...
            for _ in range(n_heads[0]):
                attns.append(
                    layers.attn_head(
                        ...
                        coef_drop=attn_drop,
                        residual=False,
                        return_coef=True  # to collect head‐specific coefficients if needed
                    )[0]
                )
                head_coef_list.append(
                    layers.attn_head(
                        inputs,
                        bias_mat=bias_mat,
                        out_sz=hid_units[0],
...
class HeteGAT(BaseGAttN):
    def inference(inputs, nb_classes, nb_nodes, training, attn_drop, ffd_drop,
                  bias_mat_list, hid_units, n_heads, activation=tf.nn.elu, residual=False,
                  mp_att_size=128, return_coef=False):
        embed_list = []
        coef_list = []
